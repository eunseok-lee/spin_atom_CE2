...
#include <math.h>
//#include <cem.h>
// This code is to calculate the correlation matrix element for 4 DEG of atom * 3 DEG of spin

void obtain_corr_mat_mag_par2(int row_ini, int row_end, double *corr_mat, int ndata, int ncorr_col, int *data, int *magmom, int c2start, int c3start, int *nlist, int *map_to_cluster2, int *map_to_cluster3, int np, int neighbor_num) {
    
    int i, j, k, l, m, n, nj, nk, tmp, tmp1;
    double chevy1a[ndata][np], chevy2a[ndata][np], chevy3a[ndata][np];
    double chevy1s[ndata][np], chevy2s[ndata][np];
    int ij_array[2], ijk_array[3];
    double phia[10], phis[4];
...
//    printf("corr_mat calculation initialization, Done, size: %d*%d \n",ndata,ncorr_col);
    ...
    for (i=0;i<ndata;i++)
        for (j=0;j<np;j++) {
            tmp = *(data+np*i+j);
            chevy1a[i][j] = sqrt(2.0/5)*tmp;
            chevy2a[i][j] = (2.0/3)*(tmp*tmp-5.0/2);
            chevy3a[i][j] = sqrt(5.0/18)*(tmp*tmp*tmp-17.0*tmp/5);
            tmp = *(magmom+np*i+j);
            ...
            if (i==ij_array[0] && j==ij_array[1]) {
                for (l=0;l<ndata;l++) {
                    ...
                    if (tmp1 >= 0) {
                        ...
                        phia[3] = 0.5*(chevy1a[l][i]*chevy2a[l][j]+chevy1a[l][j]*chevy2a[l][i]);
                        phia[4] = 0.5*(chevy1a[l][i]*chevy3a[l][j]+chevy1a[l][j]*chevy3a[l][i]);
                        phia[5] = 0.5*(chevy2a[l][i]*chevy3a[l][j]+chevy2a[l][j]*chevy3a[l][i]);
...
//                            printf("Error in three-body cluster mapping: map3(%d,%d,%d) -> %d\n",i,nj,nk,tmp1);
                            ...
                            phia[3] = (chevy1a[l][i]*chevy1a[l][j]*chevy2a[l][k]+chevy1a[l][i]*chevy2a[l][j]*chevy1a[l][k]+chevy2a[l][i]*chevy1a[l][j]*chevy1a[l][k])/3.0;
                            phia[4] = (chevy1a[l][i]*chevy1a[l][j]*chevy3a[l][k]+chevy1a[l][i]*chevy3a[l][j]*chevy1a[l][k]+chevy3a[l][i]*chevy1a[l][j]*chevy1a[l][k])/3.0;
                            phia[5] = (chevy1a[l][i]*chevy2a[l][j]*chevy2a[l][k]+chevy2a[l][i]*chevy1a[l][j]*chevy2a[l][k]+chevy2a[l][i]*chevy2a[l][j]*chevy1a[l][k])/3.0;
                            phia[6] = (chevy3a[l][i]*chevy2a[l][j]*chevy2a[l][k]+chevy2a[l][i]*chevy3a[l][j]*chevy2a[l][k]+chevy2a[l][i]*chevy2a[l][j]*chevy3a[l][k])/3.0;
                            phia[7] = (chevy1a[l][i]*chevy3a[l][j]*chevy3a[l][k]+chevy3a[l][i]*chevy1a[l][j]*chevy3a[l][k]+chevy3a[l][i]*chevy3a[l][j]*chevy1a[l][k])/3.0;
                            phia[8] = (chevy2a[l][i]*chevy3a[l][j]*chevy3a[l][k]+chevy3a[l][i]*chevy2a[l][j]*chevy3a[l][k]+chevy3a[l][i]*chevy3a[l][j]*chevy2a[l][k])/3.0;
                            phia[9] = (chevy1a[l][i]*chevy2a[l][j]*chevy3a[l][k]+chevy1a[l][i]*chevy3a[l][j]*chevy2a[l][k]+chevy2a[l][i]*chevy1a[l][j]*chevy3a[l][k]+chevy2a[l][i]*chevy3a[l][j]*chevy1a[l][k]+chevy3a[l][i]*chevy1a[l][j]*chevy2a[l][k]+chevy3a[l][i]*chevy2a[l][j]*chevy1a[l][k])/6.0;
