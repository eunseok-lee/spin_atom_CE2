...
int main(int argc, char **argv) {
    ...
    double kT;
    int np, howmanyLi, howmanyVa, howmanyMn, howmanyNi;
...
/*        strcpy(param_name,"ncorr_col");
    ...
    howmanyVa = np - howmanyLi - howmanyMn - howmanyNi;
    ncorr_col = 1 + 6*ncluster1 + 18*ncluster2 + 40*ncluster3;
    printf("ncorr_col=%d, based from ncluster1~3\n",ncorr_col);
    c2start = 6*ncluster1 + 1;      // Don't need -1: cluster2 idx starts from 0
    c3start = 18*ncluster2 + c2start;
    printf("howmanyVa = %d, ncorr_col = %d, c2start = %d, c3start = %d\n", howmanyVa, ncorr_col, c2start, c3start);

    if (rank == MASTER) {
        printf("howmanyMn=%d, howmanyNi=%d, newstart=%d, kT=%f, max_iter=%d, dispfreq=%d, howmanyLi=%d, howmanyNi=%d\n",howmanyMn, howmanyNi, newstart, kT, max_iter, dispfreq, howmanyLi, howmanyNi);
        if (numprocs>1)
...
//            printf("rand_dist[%d] = %d\n",i,rand_dist[i]);
        ...
        for (i=howmanyLi+howmanyVa;i<howmanyLi+howmanyVa+howmanyMn;i++) {
            data_trial[rand_dist[i]] = 1;
            magmom_trial[rand_dist[i]] = 0;
        }
        for (i=howmanyLi+howmanyVa+howmanyMn;i<np;i++) {
            data_trial[rand_dist[i]] = -1;
...
/*
        fp=fopen("data_trial0.dat","w");
        for (i=0;i<np;i++) {
            fprintf(fp,"%d\n",data_trial[i]);
        }
        fclose(fp);
        fp=fopen("magmom_trial0.dat","w");
        for (i=0;i<np;i++) {
            fprintf(fp,"%d\n",magmom_trial[i]);
        }
        fclose(fp);
...
//        }
        ...
        int swap_case0, swap_case1, swap_case_set[10][2];
...
//                printf("event_type = %f, RAND_MAX = %d\n",event_type,RAND_MAX);
                ...
                if (event_type < 0.25) {
//                    printf("event type 1 is running\n");
...
//                    printf("event type 2 is running\n");
                    n_swap_is = 0; n_swap_js = 0;
                    while (n_swap_is == 0 || n_swap_js == 0) {
                        i = rand() % 10;
                        swap_case0 = swap_case_set[i][0];
...
//                    printf("event type 3 is running\n");
                    n_swap_is = 0;
                    while (n_swap_is==0) {
                        i = rand() % 10;
                        swap_case0 = swap_case_set[i][0];
...
//                    printf("event type 4 is running\n");
                    n_swap_is = 0; n_swap_js = 0;
                    while (n_swap_is == 0 || n_swap_js == 0) {
                        i = rand() % 10;
                        swap_case0 = swap_case_set[i][0];
...
*/
        ...
        printf("--------------------------------------\n");
        printf("prediction for nM=%d case is done.\n", howmanyMn);
        printf("--------------------------------------\n");
...
/*
        ...
        sprintf(poscar_on_the_fly_filename,"data_trial_on_the_fly_Mn%dNi%d",howmanyMn,howmanyNi);
        ...
        sprintf(magmom_on_the_fly_filename,"magmom_trial_on_the_fly_Mn%dNi%d",howmanyMn,howmanyNi);
...
*/
    ...
    /*
        sprintf(poscarfilename,"POSCAR_Mn%dNi%d",howmanyMn,howmanyNi);
        fp = fopen(filename,"w");
        fprintf(fp,"LiMnxNi1-xO2\n");
        fprintf(fp,"1.0\n");
        ...
        else if (howmanyMn == np/2) {
            fprintf(fp,"Li Mn O\n");
            fprintf(fp,"%d %d %d\n",np/2,np/2,np);
        }
        else {
            fprintf(fp,"Li Mn Ni O\n");
            fprintf(fp,"%d %d %d %d\n",np/2,howmanyMn,maxnM-howmanyMn,np);
        }
        fprintf(fp,"Direct\n");
        for (i=0;i<np;i++)
